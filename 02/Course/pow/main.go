...
//linux: curl -l -X POST -H 'Content-type:application/json' -d '{"Result":42}' http://localhost:8080
//curl  -l -X POST -H 'Content-type:application/json' -d "{\"Result\":42}" http://localhost:8080
const difficulty = 1
...
func handleWriteBlock(w http.ResponseWriter, r *http.Request) {
	...
	mutex.Lock()
	newBlock := generateBlock(Blockchain[len(Blockchain)-1], m.Result)
	mutex.Unlock()
