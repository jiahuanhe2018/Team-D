...
func (self *NodeManager) AddConnection(c *Connection)  {
...
func (self *NodeManager) BroadcastRaw(msg string)  {
	for _, connection := range self.connections {
		connection.SendRaw(msg)
	}
}

func (self *NodeManager) BroadcastCandidateBlock(b *blockchain.Block)  {
	for _, connection := range self.connections {
		connection.SendCandidateBlock(b)
	}
}
func (self *NodeManager) BroadcastConfirmedBlock(b *blockchain.Block)  {
	for _, connection := range self.connections {
		connection.SendConfirmedBlock(b)
	}
}

func (self *NodeManager) BroadcastBlockchain(b *blockchain.Blockchain)  {
	for _, connection := range self.connections {
		connection.SendBlockchain(b)
	}
}

func (self *NodeManager) BroadcastTransaction(t *blockchain.Transaction)  {
	for _, connection := range self.connections {
		connection.SendTransaction(t)
	}
}

var NodeManagerInstance = NewNodeManager()
