...
func ReadData(rw *bufio.ReadWriter) {
	...
	for {
		...
		if str != "\n" {
			...
			//fmt.Printf("len(chain)=%d,len(BlockchainInstance.Blocks)=%d\n",len(chain),len(BlockchainInstance.Blocks))
			mutex.Lock()
			if(len(chain.Blocks)>0){
				block:=chain.Blocks[len(chain.Blocks)-1]
				for k, v := range block.Accounts {
				    _, ok := Accounts[k]
					if !ok {
						Accounts[k]=v
					}
			...
			if len(chain.Blocks) > len(BlockchainInstance.Blocks) {
				//print("===============")
				BlockchainInstance.Blocks = chain.Blocks
				BlockchainInstance.TxPool.AllTx=chain.TxPool.AllTx
			...
			} else if len(chain.Blocks) == len(BlockchainInstance.Blocks) {
                   if len(chain.TxPool.AllTx)>len(BlockchainInstance.TxPool.AllTx) {
					BlockchainInstance.TxPool.AllTx=chain.TxPool.AllTx
				   }
...
func WriteData(rw *bufio.ReadWriter) {
	...
	go func() {
		for {
			...
			mutex.Lock()
			bytes, err := json.Marshal(BlockchainInstance)//json.Marshal(BlockchainInstance.Blocks)
			if err != nil {
				log.Println(err)
			}
			mutex.Unlock()
	...
	for {
		...
		if IsBlockValid(newBlock, BlockchainInstance.Blocks[len(BlockchainInstance.Blocks)-1]) {
			//pow 内容
			mutex.Lock()
			//fmt.Printf("len(BlockchainInstance)=%d\n",len(BlockchainInstance.Blocks))
		...
		mutex.Lock()
		b, err := json.Marshal(BlockchainInstance)//json.Marshal(BlockchainInstance.Blocks)
		if err != nil {
			log.Fatal(err)
		}
		// Green console color: 	\x1b[32m
