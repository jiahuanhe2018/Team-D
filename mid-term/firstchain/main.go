...
func main() {
	...
	// //创建并打开数据库 
	// db, err := leveldb.OpenFile("D:/data.db", nil) 
	...
	config := common.NewConfig("./configuration/config.json")
	smallchain, err := boot.New(config)
	defer  smallchain.Close()
	if err != nil {
		return
	}
	go jsonrpc.StartJsonRPCServer(smallchain)
	smallchain.Start()
