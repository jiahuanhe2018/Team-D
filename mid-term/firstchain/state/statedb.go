...
type BucketTree interface {
	...
	Prepare(dirty dbtypes.WriteSet) error
	Process() (common.Hash, error)
	Commit(batch dbtypes.Batch) error
	Get(key []byte) ([]byte, error)
	Copy() *dbtypes.BucketTree
	Purge()
}
...
type txStat struct {
	...
	txIndex           uint32
	logs              map[common.Hash]dbtypes.Logs
	logSize           uint32
}
...
func New(db dbtypes.Database, root []byte) (*StateDB, error) {
	if db == nil {
		// ldb, err := dbtypes.NewLDBDataBase("/data/temp")
		// if err != nil {
		// 	return nil, err
		// }
		// db = ldb
	...
	return &StateDB{
		...
		txStat: txStat{
			logs: make(map[common.Hash]dbtypes.Logs),
		},
	}, nil
}
...
func (sdb *StateDB) Commit(batch dbtypes.Batch) (common.Hash, error) {
...
func (sdb *StateDB) AddLog(log *dbtypes.Log) {
...
func (sdb *StateDB) GetLogs(txHash common.Hash) dbtypes.Logs {
