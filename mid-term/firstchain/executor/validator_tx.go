...
func (v *TxValidator) ValidateTxs(txs dbtypes.Transactions) (valid dbtypes.Transactions, invalid dbtypes.Transactions) {
	for _, tx := range txs {
...
func (v *TxValidator) ValidateTx(tx *dbtypes.Transaction) error {
	// Check tx size
	if tx.Size() > dbtypes.MaxTxSize {
