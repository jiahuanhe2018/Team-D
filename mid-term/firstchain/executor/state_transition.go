...
type StateTransition struct {
	tx      *dbtypes.Transaction // state transition event
	vm      vm.VM
	statedb vm.StateDB
	gasPool *GasPool
}

func NewStateTransition(virtualMachine vm.VM, tx *dbtypes.Transaction) *StateTransition {
...
func ApplyTx(virtualMachine vm.VM, tx *dbtypes.Transaction) ([]byte, uint64, bool, error) {
