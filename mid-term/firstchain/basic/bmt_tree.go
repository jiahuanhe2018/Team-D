...
func NewBucketTree(db Database) *BucketTree {
	// v1.0
	return &BucketTree{
...
func (bt *BucketTree) Commit(batch Batch) error {
	if bt.db == nil {
		return ErrDbNotOpen
	}
	if !bt.dirty {
...
// Commit node store
func (bt *BucketTree) commitNode(batch Batch, node *MerkleNode) error {
	if node == nil {
		return nil
	}
	node.commit(batch)
...
func Commit(set WriteSet, db Database) error {
