...
import (
	...
	"firstchain/vm/evm/crypto"
	"firstchain/vm/evm/math"
)
...
func opBlockhash(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	n := interpreter.intPool.get().Sub(interpreter.evm.BlockHeight, Big257)
	if num.Cmp(n) > 0 && num.Cmp(interpreter.evm.BlockHeight) < 0 {
...
func opNumber(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(math.U256(interpreter.intPool.get().Set(interpreter.evm.BlockHeight)))
	return nil, nil
}
...
func makeLog(size int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		...
		interpreter.evm.StateDB.AddLog(&dbtypes.Log{
			Address: contract.Address(),
			Topics:  topics,
			Data:    d,
			// This is a non-consensus field, but assigned here because
			// core/state doesn't know the current block number.
			BlockHeight: interpreter.evm.BlockHeight.Uint64(),
		})
