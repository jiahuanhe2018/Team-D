...
type Wallet interface {
	...
	SignTx(account *Account, tx *dbtypes.Transaction) (*dbtypes.Transaction, error)
}

type SmallWallet struct {
	mu       sync.RWMutex
	accounts []*Account
	unlocked map[common.Address]*Key
}

func NewSmallWallet(accounts []*Account) *SmallWallet {
	return &SmallWallet{
		accounts: accounts,
...
func (tw *SmallWallet) Accounts() []*Account {
	tw.mu.RLock()
	defer tw.mu.RUnlock()
...
func (tw *SmallWallet) Contains(account *Account) bool {
	for _, acc := range tw.accounts {
...
func (tw *SmallWallet) CreateAccount() (*Account, error) {
	tw.mu.Lock()
	defer tw.mu.Unlock()
	acc, err := NewAccount()
	if err != nil {
...
// Lock the account
func (tw *SmallWallet) Lock(address common.Address, priv crypto.PrivKey) error {
	tw.mu.Lock()
	defer tw.mu.Unlock()
...
func (tw *SmallWallet) Unlock(address common.Address, key crypto.PrivKey) error {
	tw.mu.RLock()
...
func (tw *SmallWallet) SignHash(account *Account, hash []byte) ([]byte, error) {
...
func (tw *SmallWallet) SignTx(account *Account, tx *dbtypes.Transaction) ([]byte, error) {
	if !tw.Contains(account) {
		return nil, ErrNotFoundAcc
	}
	key, ok := tw.unlocked[account.Address]
