...
import (
	...
	"firstchain/common"
	msg "firstchain/consensus/dpos_bft/message"
	"firstchain/p2p"
	"firstchain/p2p/pb"
	"fmt"
	"time"
...
func (eg *Engine) fetchBlockLoop(seqNo uint64) (*dbtypes.Block, error) {
...
func (eg *Engine) preCommit(message *msg.ConsensusMsg) error {
	...
	if receipts, ok := eg.receipts.Load(message.SeqNo); ok {
		if err := eg.validator.ValidateState(block, eg.state, receipts.(dbtypes.Receipts)); err != nil {
...
func (eg *Engine) commit(message *msg.ConsensusMsg) error {
	...
	go eg.event.Post(&dbtypes.CommitBlockEvent{
		Block: block,
	})
	...
	case ev := <-eg.commitCompleteSub.Chan():
		// Commit complete
		if block := ev.(*dbtypes.CommitCompleteEvent).Block; block.Height() != eg.SeqNo() {
...
func (eg *Engine) checkPreCommit(block *dbtypes.Block, message *msg.ConsensusMsg) error {
	// Decode digest
...
func (eg *Engine) computeConsensusInfo(block *dbtypes.Block) (digest []byte, pubKey []byte, sign []byte, err error) {
	hash := block.Hash()
	digest = common.Sha256(hash.Bytes()).Bytes()
	sign, err = eg.Self().PrivKey().Sign(digest)
	if err != nil {
