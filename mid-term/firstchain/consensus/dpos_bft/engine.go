...
func (eg *Engine) Start() error {
	eg.newBlockSub = eg.event.Subscribe(&dbtypes.BlockReadyEvent{})
	eg.consensusSub = eg.event.Subscribe(&dbtypes.ConsensusEvent{})
	eg.receiptsSub = eg.event.Subscribe(&dbtypes.NewReceiptsEvent{})
	eg.commitCompleteSub = eg.event.Subscribe(&dbtypes.CommitCompleteEvent{})
...
func (eg *Engine) listen() {
	for {
		...
		case ev := <-eg.consensusSub.Chan():
			block := ev.(*dbtypes.ConsensusEvent).Block
...
func (eg *Engine) getReceipts(height uint64) dbtypes.Receipts {
	if receipts, exist := eg.receipts.Load(height); exist {
		return receipts.(dbtypes.Receipts)
	}
	return nil
}

func (eg *Engine) setReceipts(height uint64, receipts dbtypes.Receipts) {
	eg.receipts.Store(height, receipts)
}

// proposeBlock proposes a new block without state_root and receipts_root
func (eg *Engine) proposeBlock(txs dbtypes.Transactions) {
	...
	go eg.event.Post(&dbtypes.ProposeBlockEvent{block})
...
func (eg *Engine) process() {
	...
	if block != nil {
		go eg.event.Post(&dbtypes.ExecBlockEvent{block})
	}
}

// multicastBlock send the new proposed block to other BPs
func (eg *Engine) multicast(block *dbtypes.Block) error {
	var pids []peer.ID
...
func (eg *Engine) broadcast(block *dbtypes.Block) error {
	data, err := block.Serialize()
	if err != nil {
		return err
	}
...
func (eg *Engine) Finalize(header *dbtypes.Header, state *state.StateDB, txs dbtypes.Transactions, receipts dbtypes.Receipts) (*dbtypes.Block, error) {
