...
type ConsensusValidator interface {
	Validate(block *dbtypes.Block) error
}
...
type BlockPool struct {
	...
	valid        map[uint64]dbtypes.Blocks // Valid blocks pool. map[height]*block
...
func NewBlockPool(config *common.Config, blValidator consensus.BlockValidator, csValidator ConsensusValidator, log *logging.Logger, msgType string) *BlockPool {
	...
	bp := &BlockPool{
		...
		valid:        make(map[uint64]dbtypes.Blocks, maxBlockSize),
...
func (bp *BlockPool) GetBlock(height uint64) *dbtypes.Block {
	bp.mu.RLock()
	defer bp.mu.RUnlock()
	return bp.valid[height][0]
}

func (bp *BlockPool) GetBlocks(height uint64) dbtypes.Blocks {
	bp.mu.RLock()
	defer bp.mu.RUnlock()
	return bp.valid[height]
}

func (bp *BlockPool) isExist(block *dbtypes.Block) bool {
	bp.mu.RLock()
	defer bp.mu.RUnlock()
...
func (bp *BlockPool) AddBlock(block *dbtypes.Block) error {
	bp.mu.Lock()
	defer bp.mu.Unlock()
	if bp.isExist(block) {
...
func (bp *BlockPool) add(block *dbtypes.Block) error {
	bp.mu.Lock()
	defer bp.mu.Unlock()
