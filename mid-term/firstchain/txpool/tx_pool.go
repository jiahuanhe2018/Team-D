...
func (tp *TxPool) Start() {
	tp.newTxSub = tp.event.Subscribe(&dbtypes.NewTxEvent{})
	go tp.listen()
}
...
func (tp *TxPool) listen() {
	for {
		...
		case ev := <-tp.newTxSub.Chan():
			go tp.add(ev.(*dbtypes.NewTxEvent).Tx)
		case <-tp.quitCh:
...
func (tp *TxPool) launch(batch []interface{}) {
	go tp.event.Post(&dbtypes.ExecPendingTxEvent{
		Txs: tp.Pending(),
	})
}
...
func (tp *TxPool) Pending() dbtypes.Transactions {
	var results dbtypes.Transactions
	...
	sort.Sort(dbtypes.SortedList(results))
	return results
}

// Drop removes transactions that have been processed from tx_pool
func (tp *TxPool) Drop(drops dbtypes.Transactions) {
...
func (tp *TxPool) Add(tx *dbtypes.Transaction) error {
	return tp.add(tx)
}
...
func (tp *TxPool) addQueue(tx *dbtypes.Transaction) error {
...
func (tp *TxPool) replacePending(tx *dbtypes.Transaction) (bool, *dbtypes.Transaction) {
...
func (tp *TxPool) addPending(tx *dbtypes.Transaction) error {
	tl := tp.getPending(tx.From)
	if tl == nil {
		tl = newTxList()
...
func (tp *TxPool) postBatch(txs dbtypes.Transactions) {
	var batch []interface{}
	for _, tx := range txs {
